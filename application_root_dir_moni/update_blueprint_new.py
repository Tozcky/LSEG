...
# Send Acknowledgment Email with HTML Support
def send_ack_email(change_id: str, original_file: str, blueprint_file: str):
    ...
    <html>
    <body>
        <h2>Blueprint Update Details</h2>
        <ul>
            <li><strong>Time:</strong> {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}</li>
            <li><strong>Change ID:</strong> {change_id}</li>
            <li><strong>Original File:</strong> {original_file}</li>
            <li><strong>Blueprint File:</strong> {blueprint_file}</li>
            <li><strong>Server Details:</strong> {server_details}</li>
        </ul>
    </body>
    </html>
    """
    ...
    try:
        ...
        if CC_EMAIL:
            email_command.extend(["-c", CC_EMAIL])
